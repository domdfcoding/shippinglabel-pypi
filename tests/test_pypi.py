...
def test_get_file_from_pypi(advanced_data_regression: AdvancedDataRegressionFixture, tmp_pathplus: PathPlus):
	url = (
...
def test_get_latest():
	...
	assert get_latest("pyvoip", "3.8") == "1.6.8"
...
def test_get_sdist_url_no_files(name: str, version: str):
...
def test_get_sdist_url_no_sdist(name: str, version: str):
...
@pytest.mark.parametrize("name, version", [_param("greppy", "0.0.0"), _param("domdf_python_tools", "1.2.3")])
def test_get_wheel_url_no_version(name: str, version: str):
	with pytest.raises(InvalidRequirement, match="Cannot find version .* on PyPI."):
		get_wheel_url(name, version)


@pytest.mark.parametrize("name, version", [_param("domdf_python_toolsz", "1.2.3")])
def test_get_wheel_url_no_project(name: str, version: str):
	with pytest.raises(InvalidRequirement, match="No such project .*"):
		get_wheel_url(name, version)


@pytest.mark.parametrize("name, version", [_param("microsoft", "0.0.1")])
def test_get_wheel_url_no_files(name: str, version: str):
...
@pytest.mark.parametrize("name, version", [_param("microsoft", "0.0.1")])
def test_get_wheel_tag_mapping_no_files(name: str, version: str):
	with pytest.raises(ValueError, match=f"Version {version} has no files on PyPI."):
